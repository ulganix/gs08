...
var _PlayCanvasCompressedPlyParser = class _PlayCanvasCompressedPlyParser {
  ...
  static parseToUncompressedSplatArraySection(chunkElement, vertexElement, fromIndex, toIndex, chunkSplatIndexOffset, vertexDataBuffer, veretxReadOffset, splatArray, propertyFilter = null) {
    _PlayCanvasCompressedPlyParser.readElementData(vertexElement, vertexDataBuffer, veretxReadOffset, fromIndex, toIndex, propertyFilter);
    const { positionExtremes, scaleExtremes, position, rotation, scale, color } = _PlayCanvasCompressedPlyParser.getElementStorageArrays(chunkElement, vertexElement);
    for (let i = fromIndex; i <= toIndex; ++i) {
      const tempSplat = UncompressedSplatArray.createSplat();
...
var _INRIAV1PlyParser = class _INRIAV1PlyParser {
  ...
  parseToUncompressedSplatArraySection(header, fromSplat, toSplat, splatData, splatDataOffset, splatArray, outSphericalHarmonicsDegree = 0) {
    outSphericalHarmonicsDegree = Math.min(outSphericalHarmonicsDegree, header.sphericalHarmonicsDegree);
    for (let i = fromSplat; i <= toSplat; i++) {
      const parsedSplat = _INRIAV1PlyParser.parseToUncompressedSplat(
        splatData,
...
var PlyLoader = class _PlyLoader {
  static loadFromURL(fileName, onProgress, loadDirectoToSplatBuffer, onProgressiveLoadSectionProgress, minimumAlpha, compressionLevel, optimizeSplatData = true, outSphericalHarmonicsDegree = 0, sectionSize, sceneCenter, blockSize, bucketSize) {
...
var SplatLoader = class _SplatLoader {
  static loadFromURL(fileName, onProgress, loadDirectoToSplatBuffer, onProgressiveLoadSectionProgress, minimumAlpha, compressionLevel, optimizeSplatData = true, sectionSize, sceneCenter, blockSize, bucketSize) {
    let internalLoadType = loadDirectoToSplatBuffer ? InternalLoadType.DirectToSplatBuffer : InternalLoadType.DirectToSplatArray;
    if (optimizeSplatData) internalLoadType = InternalLoadType.DirectToSplatArray;
    const splatDataOffsetBytes = SplatBuffer.HeaderSizeBytes + SplatBuffer.SectionHeaderSizeBytes;
    const directLoadSectionSizeBytes = Constants.ProgressiveLoadSectionSize;
...
var _KSplatLoader = class _KSplatLoader {
  ...
  static loadFromURL(fileName, externalOnProgress, loadDirectoToSplatBuffer, onSectionBuilt) {
    ...
    return fetchWithProgress(fileName, localOnProgress, !loadDirectoToSplatBuffer).then((fullBuffer) => {
      ...
      const loadPromise = loadDirectoToSplatBuffer ? directLoadPromise.promise : _KSplatLoader.loadFromFileData(fullBuffer);
